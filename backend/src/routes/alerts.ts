...
router.get('/', async (req: AuthRequest, res, next) => {
  try {
    ...
    if (deviceId && typeof deviceId === 'string') where.deviceId = deviceId;
...
router.post('/bulk/acknowledge', async (req: AuthRequest, res, next) => {
  try {
    const schema = z.object({ alertIds: z.array(z.string().uuid()) });
    const { alertIds } = schema.parse(req.body);
    await prisma.alert.updateMany({ where: { id: { in: alertIds } }, data: { isAcknowledged: true, acknowledgedById: req.user!.id, acknowledgedAt: new Date() } });
    const io = req.app.get('io');
